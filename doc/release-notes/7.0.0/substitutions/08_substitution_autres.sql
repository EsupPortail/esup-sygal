...
--        pre.acteur_etablissement_id as acteur_etablissement_id_orig, -- pour debug
       ...
       pre.lib_role_compl
from pre
    left join substit_individu isub on isub.from_id = pre.individu_id --and isub.histo_destruction is null
    left join substit_etablissement esub on esub.from_id = pre.acteur_etablissement_id --and esub.histo_destruction is null
;
...
from pre
    left join substit_structure ssub on ssub.from_id = pre.structure_id --and ssub.histo_destruction is null
;
...
--        pre.unite_rech_id as unite_rech_id_orig,       -- pour debug
       ...
       pre.date_transfert
from pre
    left join substit_doctorant dsub on dsub.from_id = pre.doctorant_id --and dsub.histo_destruction is null
    left join substit_etablissement esub on esub.from_id = pre.etablissement_id --and esub.histo_destruction is null
    left join substit_ecole_doct edsub on edsub.from_id = pre.ecole_doct_id --and edsub.histo_destruction is null
    left join substit_unite_rech ursub on ursub.from_id = pre.unite_rech_id --and ursub.histo_destruction is null
;
