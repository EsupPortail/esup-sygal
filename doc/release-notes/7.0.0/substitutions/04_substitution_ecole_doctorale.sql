...
where target_table = 'ecole_doct'
  ...
  and source_table <> 'substit_ecole_doct'
...
-- create unique index ecole_doct_substit_unique_idx on substit_ecole_doct(from_id) where histo_destruction is null;
-- create unique index ecole_doct_substit_unique_hist_idx on substit_ecole_doct(from_id, histo_destruction) where histo_destruction is not null;
drop index if exists ecole_doct_substit_unique_idx;
drop index if exists ecole_doct_substit_unique_hist_idx;
create unique index ecole_doct_substit_unique_idx on substit_ecole_doct(from_id);
...
--
drop trigger if exists substit_trigger_on_ecole_doct_substit on substit_ecole_doct;
drop trigger if exists substit_trigger_on_substit_ecole_doct on substit_ecole_doct;
create trigger substit_trigger_on_substit_ecole_doct
    after insert
        --or update of histo_destruction
        or delete
    on substit_ecole_doct
    for each row
