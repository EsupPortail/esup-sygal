...
where target_table = 'unite_rech'
  ...
  and source_table <> 'substit_unite_rech'
...
(
    ...
    histo_destruction timestamp, -- les substit_xxxx ne sont pas historisés mais supprimés
    histo_destructeur_id bigint constraint unite_rech_substit_destructeur_fk references utilisateur*/
);
-- create unique index unite_rech_substit_unique_idx on substit_unite_rech(from_id) where histo_destruction is null;
-- create unique index unite_rech_substit_unique_hist_idx on substit_unite_rech(from_id, histo_destruction) where histo_destruction is not null;
create unique index unite_rech_substit_unique_idx on substit_unite_rech(from_id);
...
--
drop trigger if exists substit_trigger_on_unite_rech_substit on substit_unite_rech;
drop trigger if exists substit_trigger_on_substit_unite_rech on substit_unite_rech;
create trigger substit_trigger_on_substit_unite_rech
    after insert
        --or update of histo_destruction
        or delete
    on substit_unite_rech
    for each row
