...
class RapportService extends BaseService
{
     ...
     */
    public function computeAvailableTheseAnneeUniv(array $theseAnneeUnivs, array $rapportsExistants): array
    {
        $rapportsExistantsAnneesUnivs = array_filter(array_map(function (Rapport $rapport) {
            return $rapport->getAnneeUniv();
        }, $rapportsExistants));

        return array_filter($theseAnneeUnivs, function(TheseAnneeUniv $theseAnneeUniv) use ($rapportsExistantsAnneesUnivs) {
            return !in_array($theseAnneeUniv->getAnneeUniv(), $rapportsExistantsAnneesUnivs);
        });
    }
