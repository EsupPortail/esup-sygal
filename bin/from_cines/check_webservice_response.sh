#!/bin/sh
#
# Script de vérification du temps de réponse du web service de Facile
# GPLv2 2011
# usage :
#
# ./check_webservice_response.sh -w 1000 -c 2000 -u https://facile.cines.fr/xml
#
# émet un warning si le web service met entre 1000 et 2000ms à répondre
# un critique si le temps de réponse est supérieur à 2000ms
#
# Basé sur:
...
# Vérifie le temps de réponse su web service
webserviceload() {
    STARTTIME=$($DATE +%s%N)
	if [ -n "$NOCERT" ]; then
		$CURL --insecure --form file="@sample.pdf" $URL
	else
		$CURL -s         --form file="@sample.pdf" $URL > /dev/null 2>&1
	fi
    ENDTIME=$($DATE +%s%N)
	TIMEDIFF=$((($ENDTIME-$STARTTIME)/1000000))
	if [ "$TIMEDIFF" -lt "$WARN" ]; then 
