-- we don't know how to generate schema OTH (class Schema) :(
create sequence DOMAINE_SCIENTIFIQUE_ID_SEQ
/
...
(
  ...
  CODE VARCHAR2(64) default NULL not null
)
/
...
FOREIGN KEY (ECOLE_DOCT_ID) REFERENCES ECOLE_DOCT (ID) ON DELETE CASCADE;

ALTER TABLE THESE
  ADD CONSTRAINT THESE_UNITE_RECH_FK
FOREIGN KEY (UNITE_RECH_ID) REFERENCES UNITE_RECH (ID) ON DELETE CASCADE;
...
(
  ...
  UNITE_ID NUMBER not null
    constraint RATTACHEMENT_UNITE_ID
    references UNITE_RECH
    on delete cascade,
  ETABLISSEMENT_ID NUMBER not null
    constraint RATTACHEMENT_ETAB_ID
    references ETABLISSEMENT
    on delete cascade
)
/
...
(
  UNITE_ID NUMBER not null,
  DOMAINE_ID NUMBER not null,
  constraint SYS_C0010859
  primary key (UNITE_ID, DOMAINE_ID)
)
/
...
create view V_WF_ETAPE_PERTIN as
  ...
  FROM (
    --
    -- VALIDATION_PAGE_DE_COUVERTURE : étape toujours pertinente
    --
    SELECT
      t.id AS these_id,
      e.id AS etape_id,
...
create view V_SITU_VALIDATION_PAGE_COUV as
  SELECT
    t.id AS these_id,
    CASE WHEN v.id is not null THEN 1 ELSE 0 END valide
  FROM these t
