...
create unique index etablissement_substit_unique_idx on etablissement_substit(from_id);
...
create or replace view src_etablissement as
    ...
    where not exists (
        select from etablissement_substit sub
                  join etablissement e on e.id = sub.from_id and e.source_code = pre.source_code
