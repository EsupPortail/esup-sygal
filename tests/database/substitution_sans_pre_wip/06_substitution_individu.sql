...
create unique index individu_substit_unique_idx on individu_substit(from_id);
...
create or replace view src_individu as
    ...
    from pre
    where not exists (
        select from individu_substit sub
                  join individu i on i.id = sub.from_id and i.source_code = pre.source_code
