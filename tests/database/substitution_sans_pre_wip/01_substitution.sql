...
begin
    ...
    execute format('select count(*) from %s_substit where to_id = %s', type, p_substituant_id) into v_count;
    execute format('delete from %s_substit where to_id = %s', type, p_substituant_id);
...
begin
    ...
    execute format('insert into %s_substit (from_id, to_id, npd, histo_createur_id) values (%s, %s, %L, %s) on conflict do nothing',
...
begin
    ...
    execute format('delete from %s_substit where from_id = %s and to_id = %s returning *', type, p_substitue_id, p_substituant_id) into substit_record;
...
begin
    ...
    execute format('select * from %s_substit where from_id = %s', type, p_substitue.id) into v_substit_record;
...
begin
    ...
    if v_record.id is not null and v_record.histo_destruction is not null then
        ...
        raise exception 'Anomalie rencontrée lors de la restauration du substitué % dans % : le substitué devrait déjà être historisé', p_substitue_id, type;
    elsif v_record.id is null then
        -- le substitué n'est pas trouvé !
        raise exception 'Anomalie rencontrée lors de la restauration du substitué % dans % : le substitué est introuvable', p_substitue_id, type;
    end if;
...
begin
    ...
    elseif operation = 'INSERT' then
        ...
        --
        execute format('select to_id from %s_substit where from_id = %s', type, new.id) into substit_record;
        ...
        execute format('select * from %s_substit where npd = %L limit 1', type, v_npd) into substit_record;
...
begin
    ...
    if TG_OP = 'INSERT' then
        ...
        execute format('select count(*) from %s_substit where to_id = %s', type, old.to_id) into v_count;
        if v_count = 0 then
