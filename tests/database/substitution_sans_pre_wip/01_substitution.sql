...
begin
    ...
    perform substit_insert_log(type, 'SUBSTITUANT_HISTO', null, substituant_record_id, null,
...
begin
    ...
    perform substit_insert_log(type, 'SUBSTITUE_ADD', p_substituant_id, p_substitue_id, p_substitue_npd,
...
--drop function substit_trigger_on_fct;
create or replace function substit_trigger_on_fct() returns trigger
    language plpgsql
as
$$declare
...
begin
    ...
    if TG_OP = 'INSERT' or TG_OP = 'UPDATE' and old.histo_destruction is not null and new.histo_destruction is null then
        ...
        -- suppression du substitué (mais il continue d'exister dans la table XXX)
...
create or replace function substit_replace_foreign_keys_values(type varchar) returns void
    language plpgsql
as
$$declare
    v_cursor refcursor;
...
begin
    ...
    -- NB : Le moindre problème d'unicité stoppe le processus.
    --
    ...
    while found loop
        ...
        v_message = format('Substitution ''%s'', remplacement des FK dans %s : %s',
            upper(type), upper(v_tab_name), v_col_name);

        execute format('update %I t set %I = ss.to_id from %I ss where ss.histo_destruction is null and t.%I = ss.from_id',
            v_tab_name, v_col_name, type||'_substit', v_col_name);
...
-- drop function substit_replace_foreign_keys_values;
create or replace function substit_replace_foreign_keys_values(type varchar, p_from_id bigint, p_to_id bigint) returns void
    language plpgsql
as
$$declare
    v_cursor refcursor;
...
begin
    ...
    while found loop
        ...
        v_message = format('Substitution ''%s %s => %s'', remplacement de FK dans %s : %s = %s',
            upper(type), p_from_id, p_to_id, upper(v_tab_name), v_col_name, p_from_id);
        ...
        begin
            ...
            for v_id in execute format('update %I set %I = %s where %I = %s returning id', v_tab_name, v_col_name, p_to_id, v_col_name, p_from_id) loop
                v_count = v_count + 1;
                v_message = v_message || format(' remplacée par %s (id %s)', p_to_id, v_id);
                perform substit_insert_log(type, 'FK_REPLACE', p_from_id, p_to_id, null, v_message);
                raise notice '%', v_message;
            end loop;
            if v_count = 0 then
                v_message = v_message || ' non trouvée';
                raise notice '%', v_message;
            else
                raise notice '% remplacements effectués', v_count;
            end if;
