...
begin
    ...
    for v_data in select ss.to_id, mode() within group (order by substit_npd_structure(ps.*)) as best_npd -- NPD majoritaire
                  from structure_substit ss join structure ps on ss.from_id = ps.id
                  group by ss.to_id
        loop
...
where id in (
    ...
    left join structure_substit ss on ps.id = ss.from_id
...
select v.id
from v_etablissement_doublon v
         join etablissement pe on pe.id = v.id
...
-- Les substituants des substitutions manuelles ne doivent pas être mises à jour à partir des substituées.
update ecole_doct s set est_substituant_modifiable = false from ecole_doct_substit ss where s.id = ss.to_id ;
-- Renseignement du NPD forcé dans les substitués pour que les substitutions manuelles soient conservées.
...
select v.id
from v_ecole_doct_doublon v
         join ecole_doct pe on pe.id = v.id
...
-- Les substituants des substitutions manuelles ne doivent pas être mises à jour à partir des substituées.
update unite_rech s set est_substituant_modifiable = false from unite_rech_substit ss where s.id = ss.to_id ;
-- Renseignement du NPD forcé dans les substitués pour que les substitutions manuelles soient conservées.
...
select v.id
from v_unite_rech_doublon v
         join unite_rech pe on pe.id = v.id
...
join individu_substit sub on sub.to_id = ir.individu_id
