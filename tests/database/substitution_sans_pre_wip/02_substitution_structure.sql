...
update structure s set synchro_update_on_deleted_enabled = true from structure_substit ss where s.id = ss.from_id;
update structure s set synchro_undelete_enabled = false from structure_substit ss where s.id = ss.from_id;

-- sauvegardes tables
create table structure_sav as select * from structure;
create table structure_substit_sav as select * from structure_substit;

delete from structure_substit where histo_destruction is not null;

alter table structure_substit drop column histo_destructeur_id; -- les xxx_substit ne sont pas historisés mais supprimés
alter table structure_substit drop column histo_destruction;
...
create unique index structure_substit_unique_idx on structure_substit(from_id);
...
create or replace view src_structure as
    ...
    from pre
    where not exists (
        select from structure_substit sub
                  join structure s on s.id = sub.from_id and s.source_code = pre.source_code
