...
from pre
    join etablissement e on pre.etablissement_id = e.id
...
where not exists ( -- on ecarte les substitués
    select substit.id
    from doctorant_substit substit
    join doctorant substitue on substit.from_id = substitue.id
    where substit.histo_destruction is null and pre.source_code = substitue.source_code
);


--
-- Trigger sur la table DOCTORANT se déclenchant en cas d'insertion ou de mise à jour de l'un des attributs suivants :
...
create or replace function substit_npd_doctorant(record doctorant) returns varchar
    language plpgsql
as
$$declare
...
begin
    ...
    select substit_npd_individu(i.*) into v_npd_individu from individu i where id = record.individu_id;

    return v_npd_individu || ',' || record.ine;
end;
$$;
...
with doctorants_npd as (
    ...
    from doctorant pre
    join individu prei on prei.id = pre.individu_id and prei.histo_destruction is null
...
begin
    ...
    select count(distinct npd) into v_substit_count from v_doctorant_doublon v where v.id not in (
