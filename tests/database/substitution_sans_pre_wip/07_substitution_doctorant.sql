...
create unique index doctorant_substit_unique_idx on doctorant_substit(from_id);
...
create or replace view src_doctorant as
    ...
    where not exists (
        select from doctorant_substit sub
        join doctorant d on d.id = sub.from_id and d.source_code = pre.source_code
