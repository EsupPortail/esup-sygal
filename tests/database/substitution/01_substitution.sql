...
--
-- drop table substit_fk_replacement;
create table substit_fk_replacement(
    id bigserial primary key,
    type varchar(64),
...
create unique index substit_fk_replacement_table_name_uindex
...
begin
    ...
    execute format('insert into %s_substit (from_id, to_id, npd, histo_createur_id)
                    values (%s, %s, %L, %s)', type, p_substitue_id, p_substituant_id, p_substitue_npd, app_source_id());
...
begin
    ...
    execute format('select * from %s_substit where histo_destruction is null and from_id = %s', type, p_substitue.id) into v_substit_record;
    if v_substit_record.to_id is not null then
        ...
        if p_substitue.histo_destruction is not null then
            raise notice '=> Oui mais l''enregistrement est historisé.';
            raise notice '=> Retrait de l''enregistrement et mise à jour du substituant...';
            perform substit_remove_from_substitution(type, v_substit_record.from_id, v_substit_record.to_id);
            return true;
        end if;
        if v_substit_record.npd = p_npd then
...
begin
    ...
    v_message = format('Remplacement FK %s.%s %s => %s :', upper(p_tab_name), p_col_name, p_from_id, p_to_id);
    ...
    if v_count > 0 then
        raise notice '% % remplacements de FK.', v_message, v_count;
    end if;
...
begin
    ...
    v_message = format('Restauration FK %s.%s %s => %s:', upper(p_tab_name), p_col_name, p_to_id, p_from_id);

    for v_fkr in select * from substit_fk_replacement
                 where type = p_type and table_name = p_tab_name and column_name = p_col_name and from_id = p_from_id and to_id = p_to_id
    loop
        for v_id in execute format('update %I set %I = %s where %I = %s returning id', p_tab_name, p_col_name, p_from_id, p_col_name, p_to_id)
        loop
            v_count = v_count + 1;
    ...
    if v_count > 0 then
        delete from substit_fk_replacement
            where type = p_type and table_name = p_tab_name and column_name = p_col_name and from_id = p_from_id and to_id = p_to_id;
        raise notice '% % restaurations de FK.', v_message, v_count;
    end if;
...
begin
    ...
    elseif operation = 'INSERT' then
        ...
        -- Si c'est le cas, une erreur d'unicité dans XXXX_SUBSTIT est levée puisque l'on tente d'ajouter un substitué
        ...
        --
        execute format('select to_id from %s_substit where histo_destruction is null and from_id = %s', type, new.id) into substit_record;
        ...
        execute format('select * from %s_substit where histo_destruction is null and npd = %L and not exists (select * from %s_substit where histo_destruction is null and from_id = %s) limit 1', type, v_npd, type, new.id) into substit_record;
