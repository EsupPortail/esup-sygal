...
--
create or replace function substit_update__migrate_structure_substit() returns void
    language plpgsql
as
$$declare
    v_data record;
...
alter table pre_structure disable trigger substit_trigger_pre_structure;
select substit_update__migrate_structure_substit();
alter table pre_structure enable trigger substit_trigger_pre_structure;
/*** verif ***/
with tmp as (
select ss.id, ss.histo_creation, ss.histo_destruction, ss.npd npd_sub, ss.from_id, substit_npd_structure(ps.*) npd_calc, ps.npd_force, ss.to_id, s.histo_destruction, ps.libelle, s.libelle
...
--
-- Création des substitutions manquantes : update bidon pour déclencher le trigger.
--
alter table pre_structure enable trigger substit_trigger_pre_structure;
update pre_structure set libelle = libelle where id in (
    -- pre_structure substituables mais non substituées
    select v.id -- 578
    from v_structure_doublon v
    join pre_structure ps on ps.id = v.id
    -- substitution manquante :
    left join structure_substit ss on ps.id = ss.from_id and ss.histo_destruction is null
    where ss.to_id is null
    order by ps.id
    --
);
/* verif
...
--
select substit_replace_foreign_keys_values('structure', ss.from_id, ss.to_id)
from structure_substit ss
