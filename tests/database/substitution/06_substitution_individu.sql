...
--
alter table individu add est_substituant bool default false not null;
comment on column individu.est_substituant is 'Indique s''il s''agit d''un enregistrement substituant d''autres enregistrements';
alter table individu add est_substituant_modifiable bool default true not null;
...
-- nouvelle table PRE_INDIVIDU
create table pre_individu (like individu including all);
--alter table pre_individu disable trigger substit_trigger_pre_individu;
insert into pre_individu select * from individu;
--alter table pre_individu enable trigger substit_trigger_pre_individu;
alter table pre_individu add column npd_force varchar(256);
alter table pre_individu drop column id_ref;
alter table pre_individu add constraint pre_individu_source_fk foreign key (source_id) references source on delete cascade;
alter table pre_individu add constraint pre_individu_hc_fk foreign key (histo_createur_id) references utilisateur on delete cascade;
alter table pre_individu add constraint pre_individu_hm_fk foreign key (histo_modificateur_id) references utilisateur on delete cascade;
alter table pre_individu add constraint pre_individu_hd_fk foreign key (histo_destructeur_id) references utilisateur on delete cascade;
...
alter table pre_individu alter column id set default nextval('individu_id_seq'); -- NB : utilisation de la sequence de la table finale
...
(
    ...
    from_id bigint not null constraint individu_substit_from_fk references pre_individu on delete no action, -- 'no action' requis car trigger sur PRE_INDIVIDU
...
--drop view src_pre_individu;
create or replace view src_pre_individu as
...
--
-- Trigger sur la table PRE_INDIVIDU se déclenchant en cas d'insertion ou de mise à jour de l'un des attributs suivants :
--   - attributs participant au NPD (nom_patronymique, prenom1, date_naissance)
--   - NPD forcé.
drop trigger if exists substit_trigger_pre_individu on pre_individu;
create trigger substit_trigger_pre_individu
    after insert
        or delete
        or update of
    ...
    on pre_individu
    for each row
execute procedure substit_trigger_on_pre_fct('individu');
...
create or replace function substit_npd_individu(individu pre_individu) returns varchar
    language plpgsql
as
$$begin
    --
