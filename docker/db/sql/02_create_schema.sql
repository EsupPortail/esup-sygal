...
CREATE TABLE public.acteur (
    ...
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
...
CREATE TABLE public.attestation (
    id bigint NOT NULL,
    these_id bigint NOT NULL,
    ver_depo_est_ver_ref smallint DEFAULT 0 NOT NULL,
    ex_impr_conform_ver_depo smallint,
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    histo_createur_id bigint NOT NULL,
    histo_modification timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    histo_modificateur_id bigint NOT NULL,
    histo_destruction timestamp without time zone,
    histo_destructeur_id bigint,
    version_corrigee smallint DEFAULT 0 NOT NULL,
    creation_auto smallint DEFAULT 0 NOT NULL
);
...
CREATE TABLE public.diffusion (
    id bigint NOT NULL,
    these_id bigint NOT NULL,
    droit_auteur_ok smallint DEFAULT 0 NOT NULL,
    ...
    histo_destruction timestamp without time zone,
    histo_destructeur_id bigint,
    certif_charte_diff smallint DEFAULT 0 NOT NULL,
    confident smallint DEFAULT 0 NOT NULL,
...
CREATE TABLE public.etablissement (
    ...
    source_id bigint NOT NULL,
    source_code character varying(64) NOT NULL,
    est_membre smallint DEFAULT 0 NOT NULL,
    est_associe smallint DEFAULT 0 NOT NULL,
    est_comue smallint DEFAULT 0 NOT NULL,
    est_etab_inscription smallint DEFAULT 0 NOT NULL,
...
CREATE TABLE public.fichier_these (
    ...
    these_id bigint NOT NULL,
    est_annexe smallint DEFAULT 0 NOT NULL,
    est_expurge smallint DEFAULT 0 NOT NULL,
    est_conforme smallint,
    retraitement character varying(50),
    est_partiel smallint DEFAULT 0 NOT NULL
);
...
CREATE TABLE public.import_observ_result (
    id bigint NOT NULL,
    import_observ_id bigint NOT NULL,
    date_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    source_code character varying(64) NOT NULL,
...
CREATE TABLE public.information (
    ...
    est_visible smallint DEFAULT 1 NOT NULL,
...
CREATE TABLE public.these (
    ...
    besoin_expurge smallint DEFAULT 0 NOT NULL,
...
CREATE TABLE public.tmp_individu (
    insert_date timestamp without time zone DEFAULT LOCALTIMESTAMP,
    ...
    supann_id character varying(30),
    source_insert_date timestamp without time zone DEFAULT LOCALTIMESTAMP
);
         ...
         SELECT COALESCE(s.source_code, d.source_code) AS source_code,
            COALESCE(s.source_id, d.source_id) AS source_id,
                CASE
                    ...
                    WHEN ((s.source_code IS NOT NULL) AND (d.source_code IS NOT NULL) AND ((d.histo_destruction IS NULL) OR (d.histo_destruction > LOCALTIMESTAMP))) THEN 'update'::text
                    WHEN ((s.source_code IS NOT NULL) AND (d.source_code IS NOT NULL) AND ((d.histo_destruction IS NOT NULL) AND (d.histo_destruction <= LOCALTIMESTAMP))) THEN 'undelete'::text
                    WHEN ((s.source_code IS NULL) AND (d.source_code IS NOT NULL) AND ((d.histo_destruction IS NULL) OR (d.histo_destruction > LOCALTIMESTAMP))) THEN 'delete'::text
                    ELSE NULL::text
                END AS operation,
                CASE
           ...
           FROM ((public.individu d
             JOIN public.source src ON (((src.id = d.source_id) AND (src.importable = 1))))
...
CREATE TABLE public.role (
    ...
    attrib_auto smallint DEFAULT 0 NOT NULL,
    these_dep smallint DEFAULT 0 NOT NULL,
    histo_createur_id bigint NOT NULL,
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    histo_modificateur_id bigint,
    histo_modification timestamp without time zone,
...
CREATE MATERIALIZED VIEW public.mv_recherche_these AS
 ...
 SELECT LOCALTIMESTAMP AS date_creation,
    t.source_code AS code_these,
    d.source_code AS code_doctorant,
    ed.source_code AS code_ecole_doct,
    btrim(public.str_reduce((((((((((((((((((((((('code-ed{'::text || (eds.code)::text) || '} '::text) || 'code-ur{'::text) || (urs.code)::text) || '} '::text) || 'titre{'::text) || (t.titre)::text) || '} '::text) || 'doctorant-numero{'::text) || substr((d.source_code)::text, ("position"((d.source_code)::text, '::'::text) + 2))) || '} '::text) || 'doctorant-nom{'::text) || (id.nom_patronymique)::text) || ' '::text) || (id.nom_usuel)::text) || '} '::text) || 'doctorant-prenom{'::text) || (id.prenom1)::text) || '} '::text) || 'directeur-nom{'::text) || (a.nom_usuel)::text) || '} '::text))) AS haystack
   FROM (((((((((public.these t
     JOIN public.doctorant d ON ((d.id = t.doctorant_id)))
     JOIN public.individu id ON ((id.id = d.individu_id)))
     JOIN public.these th ON (((th.source_code)::text = (t.source_code)::text)))
...
CREATE TABLE public.origine_financement (
    ...
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    histo_createur_id bigint NOT NULL,
    histo_modification timestamp without time zone DEFAULT LOCALTIMESTAMP,
    histo_modificateur_id bigint,
    histo_destruction timestamp without time zone,
    histo_destructeur_id bigint,
    visible bigint DEFAULT 1 NOT NULL
);
...
--
-- Name: rapport_annuel; Type: TABLE; Schema: public; Owner: ad_sygal
--

CREATE TABLE public.rapport_annuel (
    id bigint NOT NULL,
    these_id bigint NOT NULL,
    fichier_id bigint NOT NULL,
    annee_univ smallint NOT NULL,
    est_final smallint DEFAULT 0 NOT NULL,
    histo_createur_id bigint NOT NULL,
    histo_modificateur_id bigint,
    histo_destructeur_id bigint,
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
...
ALTER TABLE public.rapport_annuel OWNER TO ad_sygal;

--
-- Name: rapport_annuel_id_seq; Type: SEQUENCE; Schema: public; Owner: ad_sygal
--

CREATE SEQUENCE public.rapport_annuel_id_seq
    START WITH 124
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE public.rapport_annuel_id_seq OWNER TO ad_sygal;
...
CREATE TABLE public.rdv_bu (
    ...
    mots_cles_rameau character varying(1024),
    convention_mel_signee smallint DEFAULT 0 NOT NULL,
    exempl_papier_fourni smallint,
    version_archivable_fournie smallint DEFAULT 0 NOT NULL,
    divers text,
    histo_creation timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
    histo_createur_id bigint NOT NULL,
    histo_modification timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
...
CREATE TABLE public.soutenance_membre (
    ...
    acteur_id bigint,
    visio smallint DEFAULT 0 NOT NULL,
...
CREATE TABLE public.soutenance_proposition (
    ...
    label_europeen smallint DEFAULT 0 NOT NULL,
    manuscrit_anglais smallint DEFAULT 0 NOT NULL,
    soutenance_anglais smallint DEFAULT 0 NOT NULL,
    huit_clos smallint DEFAULT 0 NOT NULL,
    exterieur smallint DEFAULT 0 NOT NULL,
...
CREATE VIEW public.src_these AS
 SELECT NULL::text AS id,
    ...
    tmp.eta_ths AS etat_these,
        CASE tmp.cod_neg_tre
            WHEN '0'::text THEN (0)::numeric
            WHEN '1'::text THEN (1)::numeric
            ELSE NULL::numeric
        END AS resultat,
...
CREATE TABLE public.synchro_log (
    id bigint NOT NULL,
    log_date timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
...
--
-- Name: v_diff_individu_indicateur; Type: VIEW; Schema: public; Owner: ad_sygal
--

CREATE VIEW public.v_diff_individu_indicateur AS
 WITH diff AS (
         SELECT COALESCE(s.source_code, d.source_code) AS source_code,
            COALESCE(s.source_id, d.source_id) AS source_id,
                CASE
...
ALTER TABLE public.v_diff_individu_indicateur OWNER TO ad_sygal;
...
--
-- Name: v_diff_role; Type: VIEW; Schema: public; Owner: ad_sygal
--
...
ALTER TABLE public.v_diff_role OWNER TO ad_sygal;
...
CREATE TABLE public.variable (
    ...
    histo_modificateur_id bigint,
    histo_modification timestamp without time zone,
    date_deb_validite timestamp without time zone DEFAULT LOCALTIMESTAMP NOT NULL,
...
--
-- Name: v_individu_role; Type: VIEW; Schema: public; Owner: ad_sygal
--

CREATE VIEW public.v_individu_role AS
 SELECT i.id AS utilisateur_id,
    i.nom_usuel,
    i.prenom1,
    i.email,
    i.source_code,
    r.role_id,
    r.id
   FROM ((public.individu_role ir
     JOIN public.individu i ON ((i.id = ir.individu_id)))
     JOIN public.role r ON ((r.id = ir.role_id)))
  ORDER BY i.nom_usuel, i.prenom1, r.role_id;


ALTER TABLE public.v_individu_role OWNER TO ad_sygal;
...
CREATE VIEW public.v_situ_archivab_va AS
 SELECT ft.these_id,
  ...
  WHERE ((ft.est_annexe = 0) AND (ft.est_expurge = 0));
...
CREATE VIEW public.v_situ_attestations_voc AS
 SELECT a.these_id,
    a.id AS attestation_id
   FROM (((public.attestation a
     JOIN public.fichier_these ft ON (((ft.these_id = a.these_id) AND (ft.est_annexe = 0) AND (ft.est_expurge = 0))))
     JOIN public.fichier f ON (((ft.fichier_id = f.id) AND (f.histo_destruction IS NULL))))
     JOIN public.version_fichier v ON (((f.version_fichier_id = v.id) AND ((v.code)::text = 'VOC'::text))))
...
CREATE VIEW public.v_situ_autoris_diff_these_voc AS
 SELECT d.these_id,
    d.id AS diffusion_id
   FROM (((public.diffusion d
     JOIN public.fichier_these ft ON (((ft.these_id = d.these_id) AND (ft.est_annexe = 0) AND (ft.est_expurge = 0))))
     JOIN public.fichier f ON (((ft.fichier_id = f.id) AND (f.histo_destruction IS NULL))))
     JOIN public.version_fichier v ON (((f.version_fichier_id = v.id) AND ((v.code)::text = 'VOC'::text))))
...
CREATE VIEW public.v_situ_depot_vo AS
 SELECT ft.these_id,
  ...
  WHERE ((ft.est_annexe = 0) AND (ft.est_expurge = 0) AND (ft.retraitement IS NULL));
...
--
-- Name: v_situ_rdv_bu_saisie_bu; Type: VIEW; Schema: public; Owner: ad_sygal
--

CREATE VIEW public.v_situ_rdv_bu_saisie_bu AS
 SELECT r.these_id,
        CASE
            WHEN ((r.version_archivable_fournie = 1) AND (r.convention_mel_signee = 1) AND (r.exempl_papier_fourni = 1) AND (r.mots_cles_rameau IS NOT NULL)) THEN 1
            ELSE 0
        END AS ok
   FROM public.rdv_bu r;


ALTER TABLE public.v_situ_rdv_bu_saisie_bu OWNER TO ad_sygal;
...
CREATE TABLE public.wf_etape (
    ...
    chemin smallint DEFAULT 1 NOT NULL,
    obligatoire smallint DEFAULT 1 NOT NULL,
        ...
        UNION ALL
         SELECT t.id AS these_id,
           ...
           FROM ((public.these t
             ...
             JOIN public.v_situ_archivab_vo situ ON (((situ.these_id = t.id) AND (situ.est_valide = 0))))
          WHERE ((t.etat_these)::text = ANY ((ARRAY['E'::character varying, 'S'::character varying])::text[]))
        UNION ALL
         SELECT t.id AS these_id,
            e.id AS etape_id,
        ...
        UNION ALL
         SELECT t.id AS these_id,
           ...
           FROM ((public.these t
             ...
             JOIN public.v_situ_archivab_va situ ON (((situ.these_id = t.id) AND (situ.est_valide = 1))))
          WHERE ((t.etat_these)::text = ANY ((ARRAY['E'::character varying, 'S'::character varying])::text[]))
        UNION ALL
         SELECT t.id AS these_id,
            e.id AS etape_id,
        ...
        UNION ALL
         SELECT t.id AS these_id,
           ...
           FROM ((public.these t
             ...
             JOIN public.v_situ_archivab_voc situ ON (((situ.these_id = t.id) AND (situ.est_valide = 0))))
          WHERE (((t.correc_autorisee IS NOT NULL) OR (t.correc_autorisee_forcee IS NOT NULL) OR ((t.correc_effectuee)::text = 'O'::text)) AND ((t.etat_these)::text = ANY ((ARRAY['E'::character varying, 'S'::character varying])::text[])))
        UNION ALL
         SELECT t.id AS these_id,
            e.id AS etape_id,
        ...
        UNION ALL
         SELECT t.id AS these_id,
           ...
           FROM ((public.these t
             ...
             JOIN public.v_situ_archivab_vac situ ON (((situ.these_id = t.id) AND (situ.est_valide = 1))))
          WHERE (((t.correc_autorisee IS NOT NULL) OR (t.correc_autorisee_forcee IS NOT NULL) OR ((t.correc_effectuee)::text = 'O'::text)) AND ((t.etat_these)::text = ANY ((ARRAY['E'::character varying, 'S'::character varying])::text[])))
        UNION ALL
         SELECT t.id AS these_id,
            e.id AS etape_id,
        ...
        UNION ALL
         SELECT t.id AS these_id,
          ...
          WHERE (((t.correc_autorisee IS NOT NULL) OR (t.correc_autorisee_forcee IS NOT NULL) OR ((t.correc_effectuee)::text = 'O'::text)) AND ((t.etat_these)::text = ANY ((ARRAY['E'::character varying, 'S'::character varying])::text[])) AND (EXISTS ( SELECT d.id,
                    ...
                    d.creation_auto
                   FROM public.diffusion d
                  WHERE ((d.these_id = t.id) AND (d.version_corrigee = 1) AND (d.autoris_mel = ANY (ARRAY[0, 1]))))))) alias38;
...
CREATE VIEW public.v_workflow AS
 SELECT t.these_id,
    ...
    dense_rank() OVER (PARTITION BY t.these_id, t.franchie ORDER BY t.ordre) AS dense_rank,
        CASE
            WHEN ((t.franchie = 1) OR (dense_rank() OVER (PARTITION BY t.these_id, t.franchie ORDER BY t.ordre) = 1)) THEN 1
            ELSE 0
        END AS atteignable,
        CASE
            WHEN ((dense_rank() OVER (PARTITION BY t.these_id, t.franchie ORDER BY t.ordre) = 1) AND (t.franchie = 0)) THEN 1
            ELSE 0
        END AS courante,
   ...
   FROM (( SELECT t_1.id AS these_id,
        ...
        UNION ALL
         SELECT t_1.id AS these_id,
            ...
            e.ordre,
                ...
                END AS franchie,
                CASE
                    WHEN ((v_1.est_valide IS NULL) OR (v_1.est_valide = 0)) THEN 0
                    ELSE 1
                END AS resultat,
            1 AS objectif
           FROM ((public.these t_1
        ...
        UNION ALL
         SELECT t_1.id AS these_id,
            ...
            e.ordre,
                ...
                END AS franchie,
                CASE
                    WHEN ((v_1.est_conforme IS NULL) OR (v_1.est_conforme = 0)) THEN 0
                    ELSE 1
                END AS resultat,
            1 AS objectif
           FROM ((public.these t_1
        ...
        UNION ALL
         SELECT t_1.id AS these_id,
            ...
            e.ordre,
                CASE
                    WHEN (v_1.est_conforme = 1) THEN 1
                    ELSE 0
                END AS franchie,
                CASE
                    WHEN (v_1.est_conforme = 1) THEN 1
